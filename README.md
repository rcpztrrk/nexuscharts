...
- Demo now supports benchmark mode for large datasets (50k+), selectable generated/CSV/WebSocket adapter data sources, and annotation snapshot save/restore controls.
...
- WASM theme sync accepts hex, RGB/RGBA, modern CSS RGB syntax, and browser-resolved CSS colors.
...
- Development demo page served from `public/`, including dataset benchmark mode, CSV/WebSocket adapter source selection, annotation snapshot controls, and Midnight/Ocean/Ember/Light/Terminal theme presets.
...
const restAdapter = createDataAdapter({
  load: async () => [{ t: 1, o: "100", h: "105", l: "98", c: "103" }],
  map: (row) => ({
    time: row.t,
    open: Number(row.o),
    high: Number(row.h),
...
chart.setAnnotations({
  priceLines: [{ price: 99.5, label: "Stop" }],
  markers: [{ time: 1, price: 0, snapTo: "low", label: "Buy", shape: "arrowUp" }]
});
